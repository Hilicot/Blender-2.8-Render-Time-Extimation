...
bl_info = {
    ...
    "wiki_url": "https://github.com/Hilicot/Blender-2.8-Render-Time-Extimation",
...
class VIEW2D_OT_RenderTimeHUD(bpy.types.Operator):
    ...
    def handle_add(self, context):
        VIEW2D_OT_RenderTimeHUD._handle = bpy.types.SpaceImageEditor.draw_handler_add(
            draw_callback_px, (self, context), 'WINDOW', 'POST_PIXEL')

    @staticmethod
    def handle_remove():
        if VIEW2D_OT_RenderTimeHUD._handle is not None:
            bpy.types.SpaceImageEditor.draw_handler_remove(VIEW2D_OT_RenderTimeHUD._handle, 'WINDOW')
        VIEW2D_OT_RenderTimeHUD._handle = None
    ...
    def modal(self, context, event):
        ...
        if timer["hud"] == False:
            VIEW2D_OT_RenderTimeHUD.handle_remove()
            return {'CANCELLED'}
    ...
    def invoke(self, context, event):
        if context.area.type == 'IMAGE_EDITOR':
            if timer["hud"] == False:
                ...
                VIEW2D_OT_RenderTimeHUD.handle_add(self, context)
                timer["hud"] = True
...
def display_hud(self, context):
    layout = self.layout
    text = VIEW2D_OT_RenderTimeHUD.bl_label if VIEW2D_OT_RenderTimeHUD._handle is None else "Hide Render Times"
    layout.operator("view2d.rendertime_hud", text=text)

# Registration

classes = (VIEW2D_OT_RenderTimeHUD,)
